...
void ParamLayer::InitBiasParam(const uint32_t param_count,
    ...
    for (uint32_t i = 0; i < param_count; ++i) {
        this->bias_.at(i) = std::make_shared<ftensor>(std::vector<uint32_t>{param_channel, param_height, param_width});
    }
}
...
void ParamLayer::InitWeightParam(const uint32_t param_count,
  ...
  for (uint32_t i = 0; i < param_count; ++i) {
    this->weights_.at(i) =
        std::make_shared<ftensor>(std::vector<uint32_t>{param_channel, param_height, param_width});
  }
}
